...
module PgSearchable
  ...
  class_methods do
    def pg_search_config(against: [], associated_against: {}, enable_multisearch: false, multisearch_label: :name)
      ...
      if enable_multisearch
        multisearchable(
          ...
          additional_attributes: ->(record) {
            record.respond_to?(multisearch_label) ? { label: record.send(multisearch_label) } : {}
          },
        )
      end
    end
